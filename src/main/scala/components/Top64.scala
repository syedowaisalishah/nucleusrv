...
class Top(programFile: Option[String], dataFile: Option[String]) extends Module {
  ...
  implicit val config: Configs = Configs(XLEN = 64, M = true, C = true, TRACE = true)

  val core: Core = Module(new Core())
  core.io.stall := false.B

  
  val dmem = Module(new SRamTop64(dataFile, dataFile))    // do for 64 bit
  val imem = Module(new SRamTop64(programFile, programFile))  // do for 64 bit
